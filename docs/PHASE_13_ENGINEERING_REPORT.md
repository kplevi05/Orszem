...
5. **Mass-assignment / HTTP-method review** — no dedicated test confirms DTOs reject unexpected fields (role/status/assignee/workflowVersion override attempts) or that unsupported HTTP methods on audit/analytics are safely rejected. **Closed — §J.2/§J.3** (also found the 500-vs-405/404 status-code finding, §X.3).
...
### J.3 Unsupported HTTP methods (brief §54)

New test class `common/UnsupportedHttpMethodIT.kt` (3 tests, all green) - Audit and Analytics are confirmed (by grepping every `@RequestMapping`, not assumed) to be entirely `@GetMapping`-only surfaces. `POST`/`PATCH`/`DELETE` against all four read-only endpoints never mutate anything (re-checked by audit-row count) and never return a `200`/`204`.

**Real, verified finding, not the brief's assumed baseline**: none of the four endpoints return Spring's usual `405 Method Not Allowed`. `ApiExceptionHandler`'s own generic `@ExceptionHandler(Exception::class)` catch-all intercepts `HttpRequestMethodNotSupportedException` before Spring's default 405 mapping ever applies, and renders it as `500 INTERNAL_ERROR` with the same fully-generic body every other unhandled exception gets. This is a genuine, pre-existing (not Phase-13-introduced) **correctness imprecision** - the wrong status code for "method not allowed" - **not a security leak**: the body is proven, not assumed, to stay the exact generic `{"code":"INTERNAL_ERROR","message":"Unexpected error.","correlationId":"..."}` shape in every case. Left unfixed per brief §54's own instruction ("if Spring already returns a safe 4xx/405: keep it... do not add custom handlers merely for aesthetics") - adding a `HttpRequestMethodNotSupportedException` handler here would be a pure status-code aesthetic change to a case that already leaks nothing, which is exactly the kind of speculative addition the brief asks to avoid. Recorded in §X.
...
`cd backend && ./gradlew build` — **BUILD SUCCESSFUL**, exit code 0. **778 tests, 0 failures, 0 skipped**, across 86 test classes — every Phase 1-12 test plus every new Phase 13 test in this report, run together, not in isolation.
...
### Screenshots (brief §63)

**No UI changed.** `git diff --stat main -- android/ web/` is empty — Phase 13's entire diff is confined to `backend/src/main` (six DTO `toString()` overrides, §G), `backend/src/test` (five new test files), and `docs/`. No compact owner regression screenshot set is attached, per the brief's own instruction to state this explicitly rather than produce screenshots of an unchanged UI.
...
3. **`POST`/`PATCH`/`DELETE` against a `@GetMapping`-only route, or a request against a path that maps to no handler at all, answers `500 INTERNAL_ERROR` rather than `405`/`404`** (found by `UnsupportedHttpMethodIT`, §J.3; the unmapped-path case additionally confirmed live in §T-W's manual verification). `ApiExceptionHandler`'s generic `Exception::class` catch-all intercepts `HttpRequestMethodNotSupportedException` and `NoHandlerFoundException` before Spring's own 405/404 mapping ever applies. A correctness imprecision, not a leak — proven, not assumed, to stay the exact generic body in both cases — and brief §54 explicitly instructs against adding a handler purely for status-code aesthetics. Accepted, left unfixed.
